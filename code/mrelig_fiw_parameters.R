...
#' 
#' # Description
#' This script summarizes published data on the abundance and per capita interaction 
#' strengths (PCIS) of mantids in order to set parameter values in the fuzzy interaction web (FIW).
#' We extracted mantid density estimates from long-established populations, 
#' calculated scaled densities, and used 
#' percentile values to inform experimental scenarios. We also calculated PCIS 
#' values from field experiments, assigning interaction strengths to 
#' functional guilds represented in the FIW.
#' 
#' # Packages
...
#' We used count data from Rathet and Hurd ([1983](https://doi.org/10.2307/2425265)) and 
#' Hurd et al. ([2020](https://doi.org/10.1093/AESA/SAZ070)) to establish a realistic range 
#' for mantid densities. Counts were averaged and scaled to reflect individuals per 1000 m².
#+ abundance_data
abun1983 <- data.frame(
  quadrat = 1:9,
...
#' PCIS values were derived from field experiments using control and mantid treatment plots. 
#' From Table 2 in Fagan and Hurd ([1991](https://doi.org/10.2307/2426113)), 
#' we extracted mean insect abundances per 1 m³ plot and applied the 
#' Paine ([1992](https://doi.org/10.1038/355073a0)) method to estimate PCIS. 
#' Four mantids were used per treatment plot.
#' From Fagan and Hurd (1994), we extracted counts from treatment plots with a 
#' moderate density of mantids (2.4 mantids per m^2^). 
#' PCIS from these studies, where predator densities were augmented, 
#' were halved to reduce possible upward bias in this predator augmentation study. 
#' 
#' Moran and Hurd ([1997](https://doi.org/10.1007/s004420050360)) used field-relevant densities of mantids, and PCIS 
